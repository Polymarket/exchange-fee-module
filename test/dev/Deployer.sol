...
library Deployer {
    ...
    function Exchange(address collateral, address ctf) public returns (address) {
        bytes memory args = abi.encode(collateral, ctf, address(0), address(0));
        address deployment = _deployCode("artifacts/Exchange.json", args);
        vm.label(deployment, "Exchange");
        return deployment;
    }
}
